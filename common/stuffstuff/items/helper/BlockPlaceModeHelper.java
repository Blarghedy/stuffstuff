...
public class BlockPlaceModeHelper
{
	...
	public static boolean onItemUseFirst(ItemStack itemstack, EntityPlayer player,
...
//		NotificationHelper.notifySelf("" + sideHit + " " + dimID + " " + charge + " " + mode + " " + item);
		...
		{
			case UP:
				if (mode == BlockPlaceMode.CREATION) y += 2; // taking advantage of this silly structure
			case DOWN:
				switch (mode)
				{
					case PILLAR:
				...
				break;
			case NORTH:
				if (mode == BlockPlaceMode.CREATION) z -= 2;
			case SOUTH:
				switch (mode)
				{
					case PILLAR:
				...
				break;
			case EAST:
				if (mode == BlockPlaceMode.CREATION) x += 2;
			case WEST:
				switch (mode)
				{
					case PILLAR:
